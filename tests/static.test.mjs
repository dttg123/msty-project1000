...
const modules=['constants.js','utils.js','state.js','portfolio.js','format.js','views.js','migration.js','toss.js'];
for (const file of modules) assert.ok(existsSync(resolve(root,'modules',file)),`missing modules/${file}`);

const index=readFileSync(resolve(root,'index.html'),'utf8');
assert.ok(index.includes('<link rel="stylesheet" href="./styles.css?v=0.9.1-r16" />'));
assert.ok(index.includes('<script type="module" src="./app.js?v=0.9.1-r16"></script>'));
...
assert.ok(sw.includes("dividend-os-v0.9.1-r16"));
assert.ok(sw.includes("cache: \'no-store\'"));
assert.ok(backup.includes("APP_VERSION = '0.9.1'"));
...
assert.ok(index.includes('app.js?v=0.9.1-r16'));
...
assert.ok(app.includes("storage.js?v=0.9.1-r16"));
...
assert.ok(app.includes("toss-client.js?v=0.9.1-r16"));
...
console.log('DividendOS v0.9.1 static QA: PASS');
