...
assert.ok(index.includes('<link rel="stylesheet" href="./styles.css?v=0.9.2-r17" />'));
assert.ok(index.includes('<script type="module" src="./app.js?v=0.9.2-r17"></script>'));
...
assert.ok(sw.includes("dividend-os-v0.9.2-r17"));
assert.ok(sw.includes("cache: \'no-store\'"));
assert.ok(backup.includes("APP_VERSION = '0.9.2'"));
...
assert.ok(index.includes('app.js?v=0.9.2-r17'));
...
assert.ok(app.includes("storage.js?v=0.9.2-r17"));
...
assert.ok(app.includes("toss-client.js?v=0.9.2-r17"));
...
console.log('DividendOS v0.9.2 static QA: PASS');
