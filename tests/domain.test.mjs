...
function testFreshWeeklyIncomeEstimate() {
  ...
  state.trades.push({id:'owned',projectId:project.id,date:'2020-01-01',type:'buy',buyType:'direct',shares:100,price:10});
  ...
  const calc=engineFor(state).computeProject(project);
  assert.equal(calc.estimateReliable,true);nearly(calc.medianDividendGapDays,7);nearly(calc.monthlyEstimate,10*52/12);
}
...
function testFutureActualRecordsAreExcluded() {
  ...
  const tomorrow=new Date();tomorrow.setUTCDate(tomorrow.getUTCDate()+2);const future=tomorrow.toISOString().slice(0,10);
