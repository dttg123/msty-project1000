...
assert.ok(index.includes('<link rel="stylesheet" href="./styles.css?v=0.11.1-r50" />'));
assert.ok(index.includes('<script type="module" src="./app.js?v=0.11.1-r50"></script>'));
...
assert.ok(sw.includes("dividend-os-v0.11.1-r50"));
assert.ok(sw.includes("cache: \'no-store\'"));
assert.ok(backup.includes("APP_VERSION = '0.11.1'"));
...
assert.ok(index.includes('app.js?v=0.11.1-r50'));
...
assert.ok(app.includes("storage.js?v=0.11.1-r50"));
...
assert.ok(app.includes("toss-client.js?v=0.11.1-r50"));
...
assert.ok(!app.includes('이전 배당 잔액 USD'));
assert.ok(!app.includes('>색상 ${index+1}<'));
assert.ok(app.includes('mergeTossSourceLedger'));
assert.ok(viewsSource.includes('<div class="upcoming-inline"'));
assert.ok(!viewsSource.includes('<button class="upcoming-inline"'));
assert.ok(app.includes('month-stacked-progress'));
assert.ok(app.includes('month-symbol-chart'));
assert.ok(app.includes('agenda-row-button'));
assert.ok(app.includes('입금 확인'));
assert.ok(!app.includes('기록 수정'));
assert.ok(readFileSync(resolve(root,'modules/state.js'),'utf8').includes('sourceLedger:{orders:[],dividends:[]}'));
...
console.log('DividendOS v0.11.1 static QA: PASS');
