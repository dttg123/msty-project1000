...
const modules=['constants.js','utils.js','state.js','portfolio.js','format.js','views.js','home-metrics.js','dividend-analytics.js','migration.js','toss.js'];
for (const file of modules) assert.ok(existsSync(resolve(root,'modules',file)),`missing modules/${file}`);

const index=readFileSync(resolve(root,'index.html'),'utf8');
assert.ok(index.includes('<link rel="stylesheet" href="./styles.css?v=0.12.7-r64" />'));
assert.ok(index.includes('<script type="module" src="./app.js?v=0.12.7-r64"></script>'));
...
assert.ok(sw.includes("dividend-os-v0.12.7-r64"));
assert.ok(sw.includes("cache: \'no-store\'"));
assert.ok(backup.includes("APP_VERSION = '0.12.7'"));
...
assert.ok(index.includes('app.js?v=0.12.7-r64'));
...
assert.ok(app.includes("storage.js?v=0.12.7-r64"));
...
assert.ok(app.includes("toss-client.js?v=0.12.7-r64"));
...
assert.ok(viewsSource.includes('data-home-cashflow-mode="year"'));
assert.ok(viewsSource.includes('data-cashflow-period='));
assert.ok(!app.includes('month-stacked-progress'));
assert.ok(!app.includes('month-symbol-chart'));
assert.ok(app.includes('month-actual-summary'));
assert.ok(app.includes('month-composition'));
...
assert.ok(refined.includes('.portfolio-section .column-chart.compact-chart{height:150px;margin-top:0}'),'portfolio chart must not exceed its fixed stage row and cover mobile axis labels');
...
console.log('DividendOS v0.12.7 static QA: PASS');
