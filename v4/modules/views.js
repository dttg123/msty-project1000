import { selectRecords, monthWeeks, historicalIncome } from './activity.js?v=0.12.7-r64';
import { APP_VERSION } from '../backup.js?v=0.12.7-r64';
import { clamp, esc, isDate, n, todayISO } from './utils.js?v=0.12.7-r64';
import { buildHomeMetrics, nextMilestone } from './home-metrics.js?v=0.12.7-r64';
import { PROJECT_CATEGORIES } from './constants.js?v=0.12.7-r64';
...
export function createViews(context) {
  ...
  function strategyInsightHTML(calc){
    if(calc.project.category==='highYield')return `<div class="analysis-divider"><strong>주당 실제 지급액</strong><span>최근 최대 8회</span></div>${paymentTrendHTML(calc)}<div class="strategy-metrics two easy-yield-metrics"><div><span>매입금 기준 1년 배당률</span><strong>${calc.analytics.trailingYoc===null?'—':fmtPct(calc.analytics.trailingYoc)}</strong></div><div><span>투입금 대비 누적 배당률</span><strong>${fmtPct(calc.lifetimeDividendRecoveryPct)}</strong></div></div><details class="metric-guide"><summary>지표 뜻 보기</summary><p><b>1년 배당률</b> 현재 보유분 매입금과 최근 1년 실제 세후 배당의 비율입니다.</p><p><b>누적 배당률</b> 직접 넣은 투자금과 지금까지 실제 세후 배당의 비율입니다. 원금 회수율은 아닙니다.</p></details>`;
    ...
    return `<div class="analysis-divider"><strong>배당 변화</strong><span>같은 기간 비교</span></div><div class="strategy-metrics two"><div><span>매입금 기준 1년 배당률</span><strong>${calc.analytics.trailingYoc===null?'—':fmtPct(calc.analytics.trailingYoc)}</strong></div><div><span>전년 같은 기간 대비</span><strong class="${calc.analytics.ytdChange===null?'':signClass(calc.analytics.ytdChange)}">${finitePct(calc.analytics.ytdChange)}</strong></div></div><details class="metric-guide"><summary>지표 뜻 보기</summary><p>최근 1년 실제 세후 배당을 내 매입금과 비교하고, 올해 받은 배당을 지난해 같은 기간과 비교합니다.</p></details>`;
  }
  ...
  function recordRow(row) {
    ...
    if(row.kind==='cash'){const withdrawal=row.purpose==='recoveryWithdrawal';title=withdrawal?'배당금 인출':esc(row.label||'배당 잔액 보정');sub=`${fmtDate(row.date)}${withdrawal?' · 원금회수 반영':''}`;value=withdrawal?`-${fmtMoney(Math.abs(n(row.amountUSD)))}`:fmtSignedMoney(row.amountUSD);cls=withdrawal?'':n(row.amountUSD)>=0?'positive':'negative';}
    ...
    page.innerHTML=`
      ...
      <div class="stack portfolio-stack">
        <article class="card portfolio-summary" style="--project-a:${colors[0]};--project-b:${colors[1]}">
          ...
          <div class="value-line"><div class="value-caption"><span>평가금액</span><small>${calc.priceAvailable?'현재가 반영':'현재가 미입력'}</small></div><strong>${calc.priceAvailable?fmtMoney(calc.marketValue):'—'}</strong><small class="${calc.priceAvailable?signClass(calc.totalReturn):''}">${calc.priceAvailable?`총손익 ${fmtSignedMoney(calc.totalReturn)}`:'현재가가 연결되면 총손익을 계산합니다.'}</small></div>
        ...
        ${p.category==='highYield'&&rec.stage!=='accumulating'?`<article class="card portfolio-section lifecycle-card"><div class="detail-title"><strong>투자 단계</strong><button class="text-link" data-goal-detail="${p.id}">목표에서 보기 ›</button></div>${lifecycleContent(calc)}</article>`:`<article class="card compact portfolio-next-goal" data-goal-detail="${p.id}" tabindex="0" role="button"><div><span>${esc(groupLabel)} · 다음 목표</span><strong>${milestone.reached?'목표 달성':`${fmtShares(milestone.shares)}주`}</strong><small>현재 ${fmtShares(calc.shares)}주${milestone.reached?'':` · ${fmtShares(milestone.remaining)}주 남음`}</small></div><b>›</b></article>`}
