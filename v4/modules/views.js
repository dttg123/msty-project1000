...
import { buildHomeMetrics, nextMilestone } from './home-metrics.js?v=0.10.0-r37';

export function createViews(context) {
  const {
    getState, getSelectedProjectId, setSelectedProjectId, getChartMode, getHistoryLimit, getCashflowMonthKey, getHomeBreakdownExpanded, getPortfolioCategory, setPortfolioCategory, getCurrentUser, isTossBridgeConfigured,
  ...
  function renderProjects() {
    ...
    const p=calc.project, colors=projectColors(p), rec=recoveryStats(calc), pct=calc.progress*100, rows=combinedRecords(calc),historyLimit=getHistoryLimit?.()||10;
    page.innerHTML=`
