...
export function createViews(context) {
  ...
  function renderHome() {
    const total=totals(),metrics=buildHomeMetrics(total.rows,state.dividends),goal=metrics.nextGoal,paceChange=metrics.pace.change,selectedMonth=metrics.months.find(item=>item.key===getCashflowMonthKey?.())||metrics.months.at(-1),twelveMonthActual=metrics.months.reduce((sum,item)=>sum+item.actual,0);
    document.getElementById('page-home').innerHTML=`${sectionTitle('홈','배당 현금흐름')}
      <div class="stack home-flow">
        <article class="card accent income-hero"><div class="card-head"><div><div class="card-kicker">이번 달 배당</div><div class="sub-number">실제 + 남은 예상 · 세후</div></div><span class="tag-pill">${new Date().getMonth()+1}월</span></div><div class="big-number home-primary-amount">${fmtMoney(metrics.month.total,0)}</div><div class="income-split"><div><span>입금 완료</span><strong>${fmtMoney(metrics.month.actual,0)}</strong></div><div><span>남은 예상</span><strong>${fmtMoney(metrics.month.remaining,0)}</strong></div>${metrics.missingEstimateCount?`<div class="forecast-missing"><span>예상 미산정</span><strong>${metrics.missingEstimateCount}종목</strong></div>`:''}</div></article>
        <article class="card compact home-row-card"><div><span>올해 받은 배당</span><small>1월부터 오늘까지</small></div><strong>${fmtMoney(metrics.year.actual,0)}</strong><div class="home-row-sub"><span>${metrics.missingEstimateCount?'연말 예상 · 일부 미산정':'연말까지 예상'}</span><b>${fmtMoney(metrics.year.total,0)}</b></div></article>
        <article class="card compact pace-card"><div class="card-head"><div><div class="card-kicker">현재 배당 페이스</div><div class="sub-number">현재 보유량 · 지급 평균 기준</div></div><span class="pace-badge ${paceChange===null?'':signClass(paceChange)}">${paceChange===null?'비교 부족':`${paceChange>=0?'+':''}${fmtPct(paceChange)}`}</span></div><div class="metric-grid"><div class="metric"><div class="metric-label">월 예상${metrics.missingEstimateCount?' · 일부 미산정':''}</div><div class="metric-value">${metrics.pace.available?fmtMoney(metrics.pace.monthly,0):'미산정'}</div></div><div class="metric"><div class="metric-label">1년 환산</div><div class="metric-value">${metrics.pace.available?fmtMoney(metrics.pace.annualized,0):'미산정'}</div></div></div></article>
        <article class="card cashflow-card"><div class="card-head cashflow-head"><div><div class="card-kicker">배당 현금흐름</div><div class="sub-number">최근 12개월 실제</div></div><strong>${fmtMoney(twelveMonthActual,0)}</strong></div>${cashflowChart(metrics.months,selectedMonth.key)}<div class="cashflow-selected"><div><span>${esc(selectedMonth.label)} 실제</span><strong>${fmtMoney(selectedMonth.actual,0)}</strong></div><div><span>남은 예상</span><strong>${fmtMoney(selectedMonth.estimated,0)}</strong></div><div><span>합계</span><strong>${fmtMoney(selectedMonth.actual+selectedMonth.estimated,0)}</strong></div></div><button class="card-link" type="button" data-income-month="${selectedMonth.key}"><span>${selectedMonth.label} 날짜별 배당 내역</span><b>›</b></button><button class="card-link" type="button" data-toggle-home-breakdown><span>이번 달 종목별 배당</span><b>${getHomeBreakdownExpanded?.()?'⌃':'⌄'}</b></button>${getHomeBreakdownExpanded?.()?`<div class="home-breakdown">${metrics.projectMonth.map(row=>`<div><strong>${esc(row.symbol)}</strong><span>실제 ${fmtMoney(row.actual,0)} · 예상 ${fmtMoney(row.remaining,0)}</span><button class="mini-icon" data-add-dividend-for="${esc(row.projectId)}" aria-label="${esc(row.symbol)} 배당 입력">입금 기록</button></div>`).join('')||'<div class="empty-inline">이번 달 배당 내역이 없습니다.</div>'}</div>`:''}</article>
        <article class="card compact next-card"><div class="card-kicker">다음 배당</div>${metrics.nextDividend?`<div class="next-line"><div><strong>${esc(metrics.nextDividend.symbol)}</strong><span>${fmtDate(metrics.nextDividend.date)} 예상</span></div><b>${fmtMoney(metrics.nextDividend.amountUSD,0)}</b></div><p>최근 지급일로 추정한 일정입니다. 확정 공시가 아닙니다.</p><button class="card-link" data-income-month="${metrics.nextDividend.date.slice(0,7)}"><span>배당 일정 · 입금 확인</span><b>›</b></button>`:'<div class="empty-inline">배당 기록을 입력하면 다음 일정을 계산합니다.</div>'}</article>
        <article class="card compact next-card ${goal?'interactive-card':''}" ${goal?`data-goal-detail="${goal.calc.project.id}" tabindex="0" role="button"`:''}><div class="card-kicker">다음 목표</div>${goal?`<div class="next-line"><div><strong>${esc(goal.calc.project.symbol)} · ${goal.milestone.reached?'목표 달성':`${fmtShares(goal.milestone.shares)}주`}</strong><span>현재 ${fmtShares(goal.calc.shares)}주${goal.milestone.reached?' · 현금흐름 단계':` · ${fmtShares(goal.milestone.remaining)}주 남음`}</span></div><b>${goal.milestone.reached?'완료':'›'}</b></div>`:'<div class="empty-inline">종목을 추가하면 가장 가까운 목표를 보여줍니다.</div>'}</article>
      </div>`;
  }
    ...
    page.innerHTML=`
      ...
      <div class="stack portfolio-stack">
        ...
        <article class="card portfolio-cashflow">
          <div class="portfolio-heading"><div><div class="card-title">배당 현금흐름</div><div class="sub-number">세후 실제 입금 기준</div></div><span class="status-pill">${frequencyOf(p).label}</span></div>
          <div class="cashflow-primary"><span>월 예상 배당</span><strong>${calc.estimateReliable?fmtMoney(calc.monthlyEstimate,0):'기록 부족'}</strong></div>
          <div class="cashflow-secondary"><div><span>이번 달 입금</span><strong>${fmtMoney(calc.currentMonthDividends,0)}</strong></div><div><span>사용 가능</span><strong class="${signClass(calc.dividendAvailable)}">${fmtMoney(calc.dividendAvailable,2)}</strong></div></div>
          ${calc.estimateStale?'<div class="tiny warning" style="margin-top:10px">기록 수·최근 지급일·지급 간격을 확인해 주세요. 예상액은 확정 배당이 아닙니다.</div>':''}
          <div class="detail-note">${!calc.estimateReliable?'보유주수와 최근 배당 기록이 있어야 예상할 수 있습니다.':calc.income?.known?'현재 보유량 × 최근 주당 세후배당':'지급 기준 주수가 없어 과거 실입금 평균 사용'}</div>${p.distributionFrequency==='weekly'?`<details class="income-evidence"><summary>배당 추세 · 최근 4회 / 8회</summary><div class="performance-rows"><div><span>최근 4회 월환산</span><strong>${calc.estimateReliable&&calc.income?.known&&calc.income.payments.length>=4?fmtMoney(calc.shortMonthlyEstimate,0):'4회 기록 필요'}</strong></div><div><span>최근 8회 월환산</span><strong>${calc.estimateReliable&&calc.income?.known&&calc.income.payments.length>=8?fmtMoney(calc.monthlyEstimate,0):'8회 기록 필요'}</strong></div><div><span>4회 평균 / 8회 평균</span><strong>${calc.estimateReliable&&calc.income?.trend!=null?fmtPct(calc.income.trend):'비교 부족'}</strong></div></div><p class="detail-note">지급일 기준 횟수입니다. 현재 주수와 분할을 반영한 주당배당으로 비교하며, ROC는 차감하지 않습니다.</p></details>`:''}<div class="entry-actions"><button class="btn primary" data-add-trade>거래</button><button class="btn secondary" data-add-dividend>배당금 입금</button></div>
        </article>
