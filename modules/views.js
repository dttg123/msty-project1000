...
import { frequencyOf } from './income.js?v=0.11.3-r53';
import { clamp, esc, isDate, n, todayISO } from './utils.js';
import { buildHomeMetrics, nextMilestone } from './home-metrics.js?v=0.11.3-r53';
    ...
    document.getElementById('page-home').innerHTML=`${sectionTitle('홈','세후 배당')}
      <div class="stack home-flow home-redesign">
        ...
        <article class="card cashflow-card history-overview">
          ...
          ${mode==='year'?`<div class="year-range-switch" aria-label="연도 표시 범위">${[['5','최근 5년'],['10','10년'],['all','전체']].map(([value,label])=>`<button class="${yearRange===value?'active':''}" data-home-year-range="${value}">${label}</button>`).join('')}</div>`:''}
