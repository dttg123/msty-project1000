...
import { buildHomeMetrics, nextMilestone } from './home-metrics.js?v=0.10.0-r33';
...
export function createViews(context) {
  ...
  function chartHTML(projectId=null) {
    ...
    return `<div class="column-chart compact-chart">${series.map(x=>`<button class="column-item" type="button" data-chart-value="${esc(fmtMoney(x.value,2))}" data-chart-label="${esc(x.label)}" aria-label="${esc(x.label)} 배당 ${esc(fmtMoney(x.value,2))}"><div class="column-value" title="${esc(fmtMoney(x.value,0))}">${esc(chartMoney(x.value))}</div><div class="column-track"><div class="column-fill" style="height:${Math.max(7,x.value/max*100)}%"></div></div><div class="column-label">${esc(x.label)}</div></button>`).join('')}</div>`;
  }
  ...
  function renderHome() {
    ...
    document.getElementById('page-home').innerHTML=`${sectionTitle('홈','배당 현금흐름')}
      <div class="stack home-flow">
        ...
        <article class="card compact pace-card"><div class="card-head"><div><div class="card-kicker">현재 배당 페이스</div><div class="sub-number">현재 보유량 · 최근 지급 기준</div></div><span class="pace-badge ${paceChange===null?'':signClass(paceChange)}">${paceChange===null?'비교 부족':`${paceChange>=0?'+':''}${fmtPct(paceChange)}`}</span></div><div class="metric-grid"><div class="metric"><div class="metric-label">월 예상</div><div class="metric-value">${fmtMoney(metrics.pace.monthly,0)}</div></div><div class="metric"><div class="metric-label">1년 환산</div><div class="metric-value">${fmtMoney(metrics.pace.annualized,0)}</div></div></div></article>
    ...
    page.innerHTML=`
      ...
      <div class="stack portfolio-stack">
        ...
        <article class="card portfolio-cashflow">
          ...
          <div class="detail-note">${calc.income?.known?'현재 보유량 × 최근 주당 세후배당':'지급 기준 주수가 없어 과거 실입금 평균 사용'}</div><div class="entry-actions"><button class="btn primary" data-add-trade>거래</button><button class="btn secondary" data-add-dividend>배당금 입금</button></div>
        </article>
